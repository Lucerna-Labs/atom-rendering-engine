...
fn circle(cx: f32, cy: f32, r: f32, ccw: bool, out: &mut Vec<PathCmd>) {
    ...
    for i in 0..n {
        ...
        let p = Vec2::new(cx + r * a.cos(), cy + r * a.sin());
        out.push(if i == 0 { PathCmd::MoveTo(p) } else { PathCmd::LineTo(p) });
    }
...
fn main() {
    ...
    let blob = vec![
        ...
        PathCmd::Cubic(Vec2::new(685.0, 135.0), Vec2::new(690.0, 255.0), Vec2::new(600.0, 300.0)),
        PathCmd::Cubic(Vec2::new(560.0, 322.0), Vec2::new(498.0, 290.0), Vec2::new(512.0, 228.0)),

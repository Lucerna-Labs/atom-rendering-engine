...
impl GpuContext {
    ...
    async fn new_async() -> Option<Self> {
        ...
        Some(Self { device, queue, bright_pipe, hblur_pipe, vblur_composite_pipe })
    }
    ...
    fn bloom(&self, fb: &mut Framebuffer, threshold: f32, sigma: f32, radius: usize) {
        ...
        let bright_params_buf = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let blur_params_buf = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
...
pub fn gpu_backend_name() -> &'static str {
    GPU.with(|ctx| {
        if ctx.is_some() { "wgpu-gpu" } else { "cpu-fallback" }
    })
}

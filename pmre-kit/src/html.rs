...
//! and document `<style>` blocks into the shared UXI box tree (`crate::ux`),
//! which then flows through the same layout + raster path as native UXI. This is the
//! "reduce": the load-bearing core is the box model, block/flex layout, inline text
//! flow, and a CSS property subset; networking and full browser semantics stay out.
//!
//! Supported structure: comments, doctype, `<script>` skipping, `<style>` rules, entities,
...
use crate::css::{self, CascadedProperty, ElementRef, ElementState, Stylesheet};
...
impl MatchNode {
    fn new(tag: &str, attrs: &HtmlAttrs) -> Self {
        Self {
...
pub fn parse(src: &str) -> UxNode {
    let (toks, style_blocks) = tokenize(src);
    let stylesheet = Stylesheet::parse(&style_blocks.join("\n"));
    let mut pos = 0usize;
...
fn tokenize(src: &str) -> (Vec<Tok>, Vec<String>) {
    let b = src.as_bytes();
    let mut i = 0usize;
    let mut out = Vec::new();
    ...
    while i < b.len() {
        if b[i] == b'<' {
            ...
            if let Some(rest) = inner.strip_prefix('/') {
                ...
                let (tag, attrs) = parse_open(inner);
                ...
                if tag == "script" || tag == "style" {
                    ...
                    let content_start = (j + 1).min(b.len());
                    let close_start = find_ascii_ci(b, content_start, close.as_bytes());
                    if tag == "style" {
                        let content_end = close_start.unwrap_or(b.len());
                        if content_start <= content_end && content_end <= src.len() {
                            style_blocks.push(src[content_start..content_end].to_string());
                        }
                    }
                    i = match close_start {
...
/// Pull the tag name and reduced attributes out of an opening-tag body, scanning
/// attributes properly (quoted values may contain spaces and `=`).
fn parse_open(inner: &str) -> (String, HtmlAttrs) {
    let mut it = inner.splitn(2, char::is_whitespace);
    let tag = it.next().unwrap_or("").to_ascii_lowercase();
    let attrs = parse_attrs(it.next().unwrap_or(""));
    (tag, attrs)
}

fn parse_attrs(attrs: &str) -> HtmlAttrs {
    let b: Vec<char> = attrs.chars().collect();
    let mut out = HtmlAttrs::default();
    let mut i = 0usize;
    while i < b.len() {
        ...
        match name.as_str() {
            ...
            "class" => {
                out.classes = value.split_whitespace().map(str::to_string).collect();
            }
            _ => {}
        }
        if name.is_empty() {
        ...
        match &toks[*pos] {
            ...
            Tok::Open {
                ...
                *pos += 1;
                ...
                nodes.push(Dom::Elem { tag, attrs, kids });
            }
        }
    }
    nodes
    ...
    for k in kids {
        match k {
            ...
            Dom::Elem {
                ...
                inline_spans(tag, attrs, inner, inh, stylesheet, path, &mut run);
                if flex_row {
                    flush(&mut run, &mut out, &mut first_flush);
                }
            }
            Dom::Elem {
                ...
                flush(&mut run, &mut out, &mut first_flush);
                let prefix = (tag == "li").then(|| "â€¢ ".to_string());
                if let Some(node) = elem_to_ux(tag, attrs, inner, inh, prefix, stylesheet, path) {
                    out.push(node);
                }
            }
        }
...
fn inline_spans(
    ...
    path.push(MatchNode::new(tag, attrs));
    let mut inh = inh;
    ...
    let css = cascaded_css(attrs, stylesheet, path);
    if !css.is_empty() {
        let mut scratch = Style::col();
        if !apply_css(&mut scratch, &mut inh, &css) {
            path.pop();
            return;
        }
    }
    for k in kids {
        match k {
            ...
            Dom::Elem { tag, attrs, kids } if is_inline(tag) && tag != "br" => {
                inline_spans(tag, attrs, kids, inh, stylesheet, path, run)
            }
...
fn elem_to_ux(
    ...
    let css = cascaded_css(attrs, stylesheet, path);
    if !css.is_empty() && !apply_css(&mut style, &mut inh2, &css) {
        path.pop();
        return None; // display:none
    }
    let flex_row = matches!(style.dir, Dir::Row);
    let children = children_to_ux(kids, inh2, li_prefix.as_deref(), flex_row, stylesheet, path);
    path.pop();
...
fn cascaded_css(attrs: &HtmlAttrs, stylesheet: &Stylesheet, path: &[MatchNode]) -> String {
    let inline = attrs
        .style
        .as_deref()
        .map(css::parse_declarations)
        .unwrap_or_default();
    let class_refs: Vec<Vec<&str>> = path
        .iter()
        .map(|node| node.classes.iter().map(String::as_str).collect())
        .collect();
    let refs: Vec<ElementRef<'_>> = path
        .iter()
        .zip(class_refs.iter())
        .map(|(node, classes)| ElementRef {
            tag: &node.tag,
            id: node.id.as_deref(),
            classes,
        ...
        .collect();
    let properties = css::cascade(stylesheet, &refs, &inline);
    cascaded_to_css(&properties)
}

fn cascaded_to_css(properties: &[CascadedProperty]) -> String {
    let mut out = String::new();
    for property in properties {
        out.push_str(&property.property);
        out.push(':');
        out.push_str(&css_value_to_text(&property.value));
        out.push(';');
    }
    out
}

fn css_value_to_text(value: &CssValue) -> String {
    match value {
        CssValue::Ident(v) | CssValue::Raw(v) | CssValue::String(v) => v.clone(),
        CssValue::Number(v) => compact_float(*v),
        CssValue::Px(v) => format!("{}px", compact_float(*v)),
        CssValue::Percent(v) => format!("{}%", compact_float(*v)),
        CssValue::Color(c) => format!(
            "rgba({},{},{},{})",
            color_channel(c.r),
            color_channel(c.g),
            color_channel(c.b),
            compact_float(c.a.clamp(0.0, 1.0))
        ),
        CssValue::Function { name, args } => format!("{name}({args})"),
    }
}

fn color_channel(v: f32) -> u8 {
...
mod tests {
    ...
    #[test]
    fn style_block_selectors_apply_to_html() {
        let doc = r#"
            <style>
              .card { padding: 12px; }
              #panel { width: 220px; }
              main > .card { margin: 3px; }
              main .label { color: #00ff00; }
            </style>
            <main><div id="panel" class="card"><span class="label">green</span></div></main>
        "#;
        let root = parse(doc);
        let UxNode::Box { children, .. } = &root else {
            panic!("main is a box")
        };
        let UxNode::Box { style, children } = &children[0] else {
            panic!("panel is a box")
        };
        assert!(matches!(style.width, Dim::Px(v) if (v - 220.0).abs() < 0.01));
        assert!((style.padding.l - 12.0).abs() < 0.01);
        assert!((style.margin.t - 3.0).abs() < 0.01);
        let UxNode::Rich { spans, .. } = &children[0] else {
            panic!("label is rich text")
        };
        assert!(
            spans[0].color.g > 0.95,
            "stylesheet descendant color applied"
        );
    }

    #[test]
    fn inline_and_important_cascade_order_apply_to_html() {
        let doc = r#"
            <style>
              .card { width: 120px !important; }
              #panel { padding: 10px; }
            </style>
            <div id="panel" class="card" style="width:80px; padding:2px"></div>
        "#;
        let root = parse(doc);
        let UxNode::Box { style, .. } = &root else {
            panic!("root is a box")
        };
        assert!(matches!(style.width, Dim::Px(v) if (v - 120.0).abs() < 0.01));
        assert!((style.padding.l - 2.0).abs() < 0.01);
    }

    #[test]
    fn stylesheet_display_none_removes_node() {
        let doc = r#"
            <style>.hidden { display:none; }</style>
            <section><div class="hidden">hidden</div><div>shown</div></section>
        "#;
        let root = parse(doc);
        let UxNode::Box { children, .. } = &root else {
            panic!("section is a box")
        };
        assert_eq!(children.len(), 1, "stylesheet display:none child dropped");
    }

    #[test]
    fn malformed_style_block_does_not_panic() {
        let root = parse("<style>.x { color red; broken</style><div class='x'>ok</div>");
        assert!(matches!(root, UxNode::Box { .. }));
    }

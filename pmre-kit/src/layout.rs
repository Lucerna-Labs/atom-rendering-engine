...
/// Convert solved layout boxes into a basic Atom IR tree without changing render behavior.
///
/// The current layout output is a flat pre-order paint list, so the IR root is a flat
/// group that preserves that order. This is a bridge for inspection/export/future render
/// paths; `pmre-orchestrator` still paints from `LaidBox` directly.
pub fn laid_boxes_to_ir(boxes: &[LaidBox]) -> AtomNode {
    AtomNode::group(boxes.iter().map(laid_box_to_ir).collect())
}

fn laid_box_to_ir(laid: &LaidBox) -> AtomNode {
    let id = laid.id.map(u64::from);
    let mut children = match &laid.kind {
        Painted::Box { .. } => {
            let mut cmds = Vec::new();
            cmds_for(laid, &mut cmds);
            cmds.into_iter()
                .map(|cmd| AtomNode::Shape {
                    id,
                    shape: cmd.shape,
                    paint: cmd.paint,
        ...
        Painted::Text {
            ...
            size,
            color,
        } => text_runs_for_box(laid.rect, id, content, *size, *color),
        Painted::Rich { spans, align } => rich_runs_for_box(laid.rect, id, spans, *align),
    };

    let mut node = if children.len() == 1 {
        children.remove(0)
    } else {
        AtomNode::Group { id, children }
    };

    if let Some(clip) = laid.clip {
        node = AtomNode::clip_rect(clip, vec![node]);
    }
    if let Some(event_id) = laid.id {
        if laid.role != Role::None {
            node = AtomNode::event_target(event_id, laid.role, vec![node]);
        }
    }
    node
}

fn text_runs_for_box(
    rect: Bounds,
    id: Option<u64>,
    content: &str,
    size: f32,
    ...
    let max_w = rect.max.x - rect.min.x;
    let line_h = size * 1.3;
    let (asc, desc) = crate::text::v_metrics(size);
    let mut y = rect.min.y;
    let mut out = Vec::new();
    for line in crate::text::wrap(content, size, max_w) {
        let origin = Vec2::new(rect.min.x, y + (line_h - (asc + desc)) * 0.5);
        out.push(AtomNode::Text {
            id,
            run: TextRun::new(line, origin, size, color),
        });
        y += line_h;
    }
    out
...
fn rich_runs_for_box(rect: Bounds, id: Option<u64>, spans: &[Span], align: Align) -> Vec<AtomNode> {
    let max_w = rect.max.x - rect.min.x;
    let (lines, line_h) = rich_lines(spans, Some(max_w));
    let mut y = rect.min.y;
    let mut out = Vec::new();
    for line in &lines {
        let x0 = match align {
            Align::Center => rect.min.x + (max_w - line.width) * 0.5,
            Align::End => rect.max.x - line.width,
            _ => rect.min.x,
        };
        let max_size = line.pieces.iter().map(|p| p.size).fold(1.0f32, f32::max);
        let (asc_l, desc_l) = crate::text::v_metrics(max_size);
        let baseline = y + (line_h - (asc_l + desc_l)) * 0.5 + asc_l;
        for piece in &line.pieces {
            let (asc_p, _) = crate::text::v_metrics_styled(piece.size, piece.bold);
            let mut run = TextRun::new(
                piece.text.clone(),
                Vec2::new(x0 + piece.x, baseline - asc_p),
                piece.size,
                piece.color,
            );
            run.bold = piece.bold;
            run.underline = piece.underline;
            out.push(AtomNode::Text { id, run });
        }
        y += line_h;
    }
    out
...
#[cfg(test)]
mod ir_conversion_tests {
    use super::*;

    #[test]
    fn laid_boxes_to_ir_preserves_order_and_event_target() {
        let boxes = vec![
            LaidBox {
                rect: Bounds {
                ...
                kind: Painted::Box {
                    background: Some(Rgba::rgb8(10, 20, 30)),
                    radius: 2.0,
                    border: None,
                    shadow: None,
                },
        ...
        let ir = laid_boxes_to_ir(&boxes);
        match ir {
            AtomNode::Group { children, .. } => {
                assert_eq!(children.len(), 2);
                assert!(matches!(
                    children[0],
                    AtomNode::EventTarget { event_id: 42, .. }
                ));
                assert!(matches!(children[1], AtomNode::Text { .. }));
            }
    ...
    #[test]
    fn laid_boxes_to_ir_wraps_clipped_nodes() {
        let clip = Bounds {
            min: Vec2::new(0.0, 0.0),
        ...
        let boxes = [LaidBox {
            rect: clip,
            kind: Painted::Box {
                background: Some(Rgba::rgb8(1, 2, 3)),
                radius: 0.0,
                border: None,
                shadow: None,
            },
        ...
        match ir {
            AtomNode::Group { children, .. } => {
                ...
                assert!(matches!(children[0], AtomNode::Clip { .. }));
            }
            other => panic!("expected root group, got {other:?}"),
        }
    }

...
|-- css            CSS stylesheet parser, selectors, specificity, cascade foundation
...
## 3.1 Atom Display List / Render IR

`pmre_kit::ir` defines the renderer-neutral Atom Display List: `AtomNode`, `TextRun`,
`ClipShape`, path stroke styles, layer blend modes, event targets, and image references.
It is pure data. It does not replace the current render path yet, and it does not choose
draw order or allocate layers.

Use `pmre_kit::layout::laid_boxes_to_ir(&boxes)` after `layout::solve` when you need a
stable scene description for inspection, export experiments, replay, or future front-end
integration. The helper preserves the current flat layout paint order and wraps current
interactive boxes as `EventTarget` nodes.
...
`<style>` stylesheet rules, void tags, malformed-input hardening (depth caps, no
quadratic scans).
...
## 5.1 CSS Parser Foundation

pmre_kit::css provides the dependency-free stylesheet parser used by the HTML front end.
It handles class, id, tag, tag.class, descendant, child, and :hover / :active /
:focus selectors, parses declarations with !important, computes specificity, matches
selectors against a lightweight element path, and exposes the cascade helper that merges
<style> rules with inline style attributes.

pmre_kit::style holds CSS value data and custom-property storage. Variables can inherit
through a VariableMap and resolve var(--name, fallback) without adding dependencies.
